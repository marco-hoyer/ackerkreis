...
export default function AdminFinancePage() {
    ...
    try {
      if (activeTab === 'overview') {
        const [statsRes, negRes] = await Promise.all([
          apiClient.get<Statistics>('/finance/statistics'),
          apiClient.get<NegativeBalance[]>('/finance/negative-balances'),
        ]);
        setStatistics(statsRes.data);
        setNegativeBalances(negRes.data);
      } else if (activeTab === 'transactions') {
        const res = await apiClient.get<TransactionsResponse>(`/finance/transactions?page=${page}&limit=50`);
        setTransactions(res.data.data);
        setTotalPages(res.data.pagination.totalPages);
      } else if (activeTab === 'unmatched') {
        const res = await apiClient.get<TransactionsResponse>(`/finance/unmatched?page=${page}&limit=50`);
        setTransactions(res.data.data);
        setTotalPages(res.data.pagination.totalPages);
      }
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await apiClient.post<{ imported: number; matched: number }>('/finance/import', { csv: csvContent });
      setImportResult(res.data);
      toast({ title: `${res.data.imported} Transaktionen importiert, ${res.data.matched} zugeordnet` });
      setCsvContent('');
    } catch (error: any) {
      toast({
        variant: 'destructive',
  ...
  const handleManualMatch = async (transactionId: string, subscriptionId: string) => {
    try {
      await apiClient.patch(`/finance/transactions/${transactionId}/match`, { subscriptionId });
