...
import { openVotingRoundAction, closeVotingRoundAction } from '@/lib/actions/voting';
import { Plus, Play, Square, Target, Users, BarChart3 } from 'lucide-react';
...
export default function AdminVotingPage() {
  ...
  const loadStats = useCallback(async (roundId: string) => {
    try {
      const res = await fetch(`/api/voting/rounds/${roundId}/stats`);
      if (res.ok) {
  ...
  const loadRounds = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/voting/rounds');
      if (res.ok) {
        const data: VotingRound[] = await res.json();
        setRounds(data);
        if (data.length > 0) {
  ...
  const handleOpen = async (id: string) => {
    try {
      await openVotingRoundAction(id);
      toast({ title: 'Abstimmungsrunde geoeffnet' });
      loadRounds();
    } catch (error: unknown) {
      toast({ variant: 'destructive', title: 'Fehler', description: error instanceof Error ? error.message : 'Fehler' });
    }
  };
  ...
  const handleClose = async (id: string) => {
    ...
    try {
      await closeVotingRoundAction(id);
